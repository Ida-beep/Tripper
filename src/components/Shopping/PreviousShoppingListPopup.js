...
function PreviousShoppingListPopup(props) {
    ...
    useEffect(()=> {
        if (typeof props.excursionID !== "undefined") {
            async function fetchData(){
                setShoppingItems(await ShoppingAPI.fetchPreviousShoppingListFromDB(props.excursionID.id))
            };
            fetchData();
    ...
    const buttons = [
        ...
        <button className="button-primary-extra-small" disabled={disable()}>Add Selected</button>,
        <button className="button-secondary-extra-small" onClick={props.editState}>Finish</button>
    ]

    return (props.trigger) && (
        <PopUp  title={props.title} data={shoppingItems} editState={props.editState}
            submitChanges={handleSubmit} buttons={buttons}>
            <div style={{marginTop:"20px"}} className="card-container">
                <div className="table-container">
                        <TableScaffold onSelection={(selectedItem)=>addElementToSelected(selectedItem)}
                            tkey={[
                                "itemName",
                                "amount",
                                "unit"
